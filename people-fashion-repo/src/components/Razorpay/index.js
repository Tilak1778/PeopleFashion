...
function Razorpay({ updatePayment, handleSubmit, address, totalAmount }) {
	...
	async function displayRazorpay() {
		try {
			...
			console.log("tilak: local host post before");
			const result = await axios.post('http://172.174.121.231/payment/orders', {
				amount: totalAmount,
			});
			const { amount, id: order_id, currency } = result.data;

			const options = {
				key: 'rzp_test_4tjk8nyEQCMQYE', // Enter the Key ID generated from the Dashboard
				...
				handler: async function (response) {
					...
					try {
						const result = await axios.post(
							'http://172.174.121.231/payment/orders',
							data
						);
